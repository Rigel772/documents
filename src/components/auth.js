...
export async function saveCurrentCategory(new_categ_name) {
  ...
  return docRef
    ...
    .then(function () {
      console.log("Current category saved", new_categ_name);
    })
    .catch(function (error) {
      // The document probably doesn't exist.
      console.error("Error saving current catrgory: ", error);
    });
}
...
export const updateEvent = (category, event, id) => {
  let eventToUpdate = db.collection(category).doc(id);
  console.log(id)
